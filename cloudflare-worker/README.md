...
3. 准备 Cloudflare Token、Account ID、Fork 后的仓库地址和魔方财务 API；如需网页一键更新，再准备 GitHub Fine-grained Token：打开 <https://github.com/settings/personal-access-tokens/new>，仓库只选择你的 Fork 仓库，权限设置 **Actions: Read and write**、**Contents: Read-only**，生成后复制 `github_pat_` 开头的令牌。
